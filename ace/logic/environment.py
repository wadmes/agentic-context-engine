...
class TesterResponse:
    """Single tester response entry following the JSON schema."""

    input_patterns: List[Mapping[str, int]]
    good_outputs: List[Mapping[str, int]]
    faulty_outputs: List[Mapping[str, int]]
...
class LogicDiagnosisEnvironment(TaskEnvironment):
    ...
    @staticmethod
    def load_tester_responses(path: str | Path) -> Iterable[TesterResponse]:
        """Load tester responses from a JSON array following the required schema."""
        ...
        for index, item in enumerate(payload):
            if not isinstance(item, Mapping):
                raise ValueError(f"Tester response at index {index} must be an object.")
            try:
                responses.append(
                    TesterResponse(
                        input_patterns=_coerce_pattern_list(item.get("input_patterns"), index, "input_patterns"),
                        good_outputs=_coerce_pattern_list(item.get("good_outputs"), index, "good_outputs"),
                        faulty_outputs=_coerce_pattern_list(item.get("faulty_outputs"), index, "faulty_outputs"),
                        differences=_coerce_differences(item.get("differences"), index),
                    )
                )
            except ValueError as exc:
                raise ValueError(f"Invalid tester response at index {index}: {exc}") from exc
...
def _coerce_pattern_list(
    value: Optional[Iterable[Mapping[str, int]]],
    index: int,
    field_name: str,
) -> List[Mapping[str, int]]:
    if value is None:
        raise ValueError(f"Missing '{field_name}' array for tester response {index}")
    if not isinstance(value, list):
        raise ValueError(
            f"'{field_name}' must be a list of mappings for tester response {index}"
        )
    normalised: List[Mapping[str, int]] = []
    for entry_pos, entry in enumerate(value):
        if not isinstance(entry, Mapping):
            raise ValueError(
                f"'{field_name}' entry {entry_pos} is not an object for tester response {index}"
            )
        normalised.append({str(k): int(v) for k, v in entry.items()})
    return normalised


def _coerce_differences(value: Optional[Iterable[str]], index: int) -> List[str]:
    if value is None:
        raise ValueError(f"Missing 'differences' array for tester response {index}")
    if not isinstance(value, list):
        raise ValueError(
            f"'differences' must be a list of strings for tester response {index}"
        )
